...
public class SetupCommand implements CommandExecutor {
		...
		public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
			...
			if (args.length==0) {
			...
			} else if (args[0].equalsIgnoreCase("setregisterlobby"))  {
				if (args.length==1) {
					Main.arenas.getConfig().set("registerlobby", Utils.getUtils().setLoc(p.getLocation(), true));
					Main.arenas.saveConfig();
				}
			} else if (args[0].equalsIgnoreCase("setarena1")) {
				if (args.length==2) {
					p.sendMessage("§aYou have set the location of Arena #1 on arena §b"+ args[1]);
				...
				if (args.length==2) {
					p.sendMessage("§aYou have set the location of Arena #2 on arena §b"+ args[1]);
				...
				if (args.length==2) {
					p.sendMessage("§aYou have set the location of Spawn #1 on arena §b"+ args[1]);
				...
				if (args.length==2) {
					p.sendMessage("§aYou have set the location of Spawn #2 on arena §b"+ args[1]);
				...
				if (args.length==3) {
					...
					p.sendMessage("§aYou have set the min amount of players on arena §b"+ args[1]);
				...
				if (args.length==3) {
					...
					p.sendMessage("§aYou have set the max amount of players on arena §b"+ args[1]);
					Main.arenas.getConfig().set("arenas."+args[1]+".max_size", max);
					Main.arenas.saveConfig();
				} else {
					
				}
			}  else if (args[0].equalsIgnoreCase("setradious")) {
				if (args.length==3) {
					int max = 0;
					try {
						max = Integer.parseInt(args[2]);			
					...
					p.sendMessage("§aYou have set the radious of the arena §b"+ args[1]);
					Main.arenas.getConfig().set("arenas."+args[1]+".radious", max);
					Main.arenas.saveConfig();
				} else {
				...
				if (args.length==2) {
					p.sendMessage("§aYou have set the location of Lobby on arena §b"+ args[1]);
				...
				if (args.length==2) {
					p.sendMessage("§aYou have set the item to §b"+p.getItemInHand().getType()+ "§a on arena §b"+ args[1]);
				...
				if (args.length==2) {
					p.sendMessage("§aYou have set the location of Main Spawn on arena §b"+ args[1]);
				...
				if (args.length==3) {
					...
					p.sendMessage("§aYou have set the location of Type on arena §b"+ args[1]);
					...
					if (Main.arenas.getConfig().contains("arenas."+args[1]+".gametype")) {
						...
						if (gametype.equals(GameType.FFA)) {
							if (!Main.arenas.getConfig().contains("arenas."+args[1]+".arena1") 
									...
									|| !Main.arenas.getConfig().contains("arenas."+args[1]+".min_size")
									|| !Main.arenas.getConfig().contains("arenas."+args[1]+".max_size")
									|| !Main.arenas.getConfig().contains("arenas."+args[1]+".radious")
									){
								p.sendMessage("§cThe arena §b" + args[1] + " §cdoesnt have all locations set.");
								return false;
							}
						...
						int min = Main.arenas.getConfig().getInt("arenas."+args[1]+".min");
						int max = Main.arenas.getConfig().getInt("arenas."+args[1]+".max");
						
						if (gametype.equals(GameType.FFA)) {	
							int radious = Main.arenas.getConfig().getInt("arenas."+args[1]+".radious");
							Location mainspawn = Utils.getUtils().getLoc(Main.arenas.getConfig().getString("arenas."+args[1]+".mainspawn"));
						DataManager.getManager().loadArena(args[1], mainspawn,null,null, lobby, arena1, arena2,spleeftype,gametype,null,min,max,null,null,radious);
							...
							DataManager.getManager().loadArena(args[1], null,spawn1,spawn2,lobby, arena1, arena2,spleeftype,gametype,item,min,max,null,null,0);
							...
							Location spawn2 = Utils.getUtils().getLoc(Main.arenas.getConfig().getString("arenas."+args[1]+".spawn2"));
							Material item = Material.valueOf(Main.arenas.getConfig().getString("arenas."+args[1]+".item"));
							DataManager.getManager().loadArena(args[1], null,spawn1,spawn2,lobby, arena1, arena2,spleeftype,gametype,item,min,max,flag1,flag2,0);
						}
